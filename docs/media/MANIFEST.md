...
| --- | ---: | --- | --- |
| `demo/apk-scope-ca-full-flow.mp4` | 175.8s | 720×1600 H.264, no audio | **Primary CA demo.** One continuous run: APK selection → CA Mode → static analysis → disposable Managed Work Profile provisioning → sandbox prepare/install → Live Monitor VPN evidence → Traffic Inspector with the Work Profile CA installed and decryption enabled, showing a decoded WSS `101` handshake with bidirectional frames and full HTTPS `200` request/response bodies. CA **was** installed on camera, so plaintext HTTPS/WSS decryption is demonstrated. Embedded in the README as a native player via a GitHub user-attachments URL; the MP4 here is the downloadable source. |
| `demo/apk-scope-frida-full-flow.mp4` | 145.0s | 720×1600 H.264, no audio | **Primary Frida demo.** One continuous run: APK selection → Frida Mode → gadget patch pipeline (re-signed APK) → static analysis of the patched APK → sandbox prepare/install → patched app launch → gadget `SSL_read`/`SSL_write` chunks stream into APK Scope's receiver (Connected package + chunk count) → Traffic Inspector shows decoded HTTPS `200` transactions. No CA installed. gzip-encoded response bodies appear as raw compressed bytes; headers and uncompressed bodies decode cleanly. Development POC, not the default workflow. Embedded in the README as a native player via a GitHub user-attachments URL; the MP4 here is the downloadable source. |
